            ...
            if (!isValidBlock)
                ...
                if (jobManagerLastSubmitBlockHex === blockHex) {
                    ...
                    SubmitBlock(blockHex, function () {
						// in SIN blockhash is x22i and mining hash is x25x so different treatment is needed to accept block
						var data = Buffer.from(blockHex, "hex");
						var hashedData = multiHashing['x22i'](data);
						
                        CheckBlockAccepted(hashedData.toString('hex'), function (isAccepted, tx) {
